...
def runner(date_of_url_data, pldata_to_load, npz_to_load, world_scene_video_path, export_file_path, gaze_window_size_ms=GAZE_WINDOW_SIZE_MS, polynomial_grade=POLYNOMIAL_GRADE, min_vel_thresh=MIN_VEL_THRESH, gain_factor=GAIN_FACTOR):
    ...
    v_hat = np.array([fixation_packages.gaze_processing.calculateGazeVelocity(savgol_gaze_vec[0], gaze_timestamp), fixation_packages.gaze_processing.calculateGazeVelocity(savgol_gaze_vec[1], gaze_timestamp)])
    ...
    print(f"Global vec list len: {len(global_vec_list)}")
