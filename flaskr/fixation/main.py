...
def runner(date_of_url_data, pldata_to_load, npz_to_load, world_scene_video_path, export_file_path, gaze_window_size_ms=GAZE_WINDOW_SIZE_MS, polynomial_grade=POLYNOMIAL_GRADE, min_vel_thresh=MIN_VEL_THRESH, gain_factor=GAIN_FACTOR):
    ...
    gaze_timestamp = np.mean([gaze_data_dict["left_timestamps"][0:min_len], gaze_data_dict["right_timestamps"][0:min_len]], axis=0)
    ...
    v_hat = np.array(np.column_stack([fixation_packages.gaze_processing.calculateGazeVelocity(savgol_gaze_vec[0], gaze_timestamp), fixation_packages.gaze_processing.calculateGazeVelocity(savgol_gaze_vec[1], gaze_timestamp)]))
    # print(v_hat.shape)
    # input()
    # print('START')
...
def main():
    print("starting")
    runner(date_of_url_data=DATE_OF_URL_DATA, pldata_to_load=PLDATA_TO_LOAD, npz_to_load=NPZ_TO_LOAD, world_scene_video_path='./test_data/videos/video.mp4', export_file_path="export.json", gaze_window_size_ms=GAZE_WINDOW_SIZE_MS, polynomial_grade=POLYNOMIAL_GRADE, min_vel_thresh=MIN_VEL_THRESH, gain_factor=GAIN_FACTOR)
    print("complete")
