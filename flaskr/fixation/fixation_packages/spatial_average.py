...
def resample(stream, target_hz):
    ...
    def resample_stream(t_orig, data_orig):
        return np.vstack([
            np.interp(new_timestamps, t_orig, data_orig[:, dim])
            for dim in range(data_orig.shape[1])
        ]).T

    new_data1 = resample_stream(t1, data1)

    return new_timestamps, new_data1